...
router.get("/", checkPermission('attendance'), async (req, res) => {   // ✅ ADD checkPermission
  try {
...
router.post("/", checkPermission('attendance'), async (req, res) => {   // ✅ ADD checkPermission
  try {
...
router.get("/employee-summary/:employeeId", checkPermission('attendance'), async (req, res) => {   // ✅ ADD checkPermission
  try {
    ...
    res.json({ success: true, data: { presentDays, absentDays, halfDays, totalDays } });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
